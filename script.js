...
function studentWeakTopic(student){
  const topics=student.topics||{};
  const entries=Object.keys(topics).map(key=>{
    const t=topics[key];
    return {key, label:TOPIC_LABELS[key]||key, attempts:t.attempts||0, correct:t.correct||0, pct:t.attempts?Math.round((t.correct||0)/t.attempts*100):0};
  }).filter(t=>t.attempts>=3 && t.pct<70).sort((a,b)=>a.pct-b.pct);
  return entries[0]||null;
}
function classWeakTopics(students){
  const totals={};
  students.forEach(s=>{
    const topics=s.topics||{};
    Object.keys(topics).forEach(key=>{
      totals[key]=totals[key]||{attempts:0,correct:0};
      totals[key].attempts+=topics[key].attempts||0;
      totals[key].correct+=topics[key].correct||0;
    });
  });
  return Object.keys(totals).map(key=>{
    const t=totals[key];
    return {key, label:TOPIC_LABELS[key]||key, attempts:t.attempts, correct:t.correct, pct:t.attempts?Math.round(t.correct/t.attempts*100):0};
  }).filter(t=>t.attempts>=3).sort((a,b)=>a.pct-b.pct);
}
function renderClassAnalytics(code,data){
  ...
  const medals=["🥇","🥈","🥉"];
  const topScores=[...students].sort((a,b)=>(b.score||0)-(a.score||0)).slice(0,5);
  const topStreaks=[...students].filter(s=>(s.bestStreak||0)>0).sort((a,b)=>(b.bestStreak||0)-(a.bestStreak||0)).slice(0,5);
  const leaderboardHtml=students.length?`<div class="leaderboard-grid"><div class="leaderboard-col"><h5>⭐ Top Scores</h5>${topScores.length?`<ol class="leaderboard-list">${topScores.map((s,i)=>`<li class="leaderboard-row ${i<3?'top3':''}"><span class="leaderboard-rank">${medals[i]||(i+1)}</span><span class="leaderboard-name">${escapeHtml(s.name||"Student")}</span><span class="leaderboard-value">${s.score||0} pts</span></li>`).join("")}</ol>`:'<p class="leaderboard-empty">No scores yet.</p>'}</div><div class="leaderboard-col"><h5>🔥 Top Streaks</h5>${topStreaks.length?`<ol class="leaderboard-list">${topStreaks.map((s,i)=>`<li class="leaderboard-row ${i<3?'top3':''}"><span class="leaderboard-rank">${medals[i]||(i+1)}</span><span class="leaderboard-name">${escapeHtml(s.name||"Student")}</span><span class="leaderboard-value">${s.bestStreak||0} streak</span></li>`).join("")}</ol>`:'<p class="leaderboard-empty">No streaks recorded yet.</p>'}</div></div>`:'';
  const clsWeak=classWeakTopics(students).slice(0,3);
  const weakTopicsHtml=`<div class="class-weak-topics">${clsWeak.length?`<p class="weak-topics-summary">Across the class, students are weakest at <strong>${clsWeak.map(t=>`${t.label} (${t.pct}%)`).join(', ')}</strong>.</p>`:`<p class="weak-topics-summary">Not enough quiz/practice data yet to detect a class-wide weak topic.</p>`}</div>`;
  box.innerHTML=`<h3>📊 ${escapeHtml(meta.name||code)} — LIVE Dashboard</h3><p>Class code: <strong>${code}</strong> · ${students.length} student(s) · updates automatically</p><div class="analytics-grid"><div class="analytics-stat"><span>Accuracy</span><strong>${accuracy}%</strong></div><div class="analytics-stat"><span>Average Score</span><strong>${avgScore}</strong></div><div class="analytics-stat"><span>Students</span><strong>${students.length}</strong></div><div class="analytics-stat"><span>Completion</span><strong>${students.length?Math.round(completed/students.length*100):0}%</strong></div></div><h4>🏆 Class Leaderboard</h4>${leaderboardHtml}<h4 style="margin-top:18px">🔍 Weak Topics</h4>${weakTopicsHtml}<h4 style="margin-top:18px">👥 Student Performance</h4><table class="analytics-table"><thead><tr><th>Student</th><th>Accuracy</th><th>Score</th><th>Best Streak</th><th>Weak Topic</th><th>Completion</th><th>Last Activity</th></tr></thead><tbody>${students.length?students.map(s=>{const w=studentWeakTopic(s);return `<tr><td>${escapeHtml(s.name||"Student")}</td><td>${s.attempts?Math.round((s.correct||0)/s.attempts*100):0}%</td><td>${s.score||0}</td><td>${s.bestStreak||0}</td><td>${w?`<span class="weak-topic-tag">⚠️ ${escapeHtml(w.label)} (${w.pct}%)</span>`:'<span class="weak-topic-none">—</span>'}</td><td>${s.completed?"✅ Complete":"⏳ In progress"}</td><td>${s.lastActivity?new Date(s.lastActivity).toLocaleTimeString():"—"}</td></tr>`;}).join(""):'<tr><td colspan="7">Waiting for students to join...</td></tr>'}</tbody></table><h4 style="margin-top:18px">⚠️ Common Mistakes</h4><div class="mistake-list">${common.length?common.map(([p,n])=>`<span class="mistake-tag">${escapeHtml(p)} × ${n}</span>`).join(""):"<span>No mistakes recorded yet.</span>"}</div>`;
}
...
async function joinClass(){
  ...
  try{
    ...
    await ref.child("students/"+studentId).update({name,score:0,correct:0,attempts:0,bestStreak:0,completed:false,joinedAt:firebase.database.ServerValue.TIMESTAMP,lastActivity:firebase.database.ServerValue.TIMESTAMP});
...
async function recordClassAttempt(isCorrect){
  ...
  try{
    ...
    await sref.transaction(s=>{s=s||{name:activeStudentName,score:0,correct:0,attempts:0}; s.name=activeStudentName;s.attempts=(s.attempts||0)+1;if(isCorrect){s.correct=(s.correct||0)+1;s.score=(s.score||0)+10;}s.bestStreak=Math.max(s.bestStreak||0,bestStreak);s.lastActivity=now;if(meta.questions&&s.attempts>=meta.questions)s.completed=true;return s;});
...
function submitAnswer() {
  ...
  recordTopicAttempt((minuend < 0 || subtrahend < 0) ? 'negative' : 'basic', isCorrect);
...
function setQuizDifficulty(level){quizState.difficulty=level;playSound('click');document.querySelectorAll('.quiz-difficulty').forEach(b=>b.classList.toggle('active',b.dataset.level===level));renderQuizPreview();const startBtn=document.getElementById('quizStartBtn');if(startBtn)startBtn.textContent=`START ${quizDifficultyLabel(quizState.difficulty).toUpperCase()} QUIZ`;}
...
// ============================================================
// WEAK TOPICS TRACKER — tracks per-topic accuracy across Practice,
// Mission Mode, Word Problems, Time Trial, and Quiz Challenge, and
// surfaces the student's weakest topics on the Progress screen.
// ============================================================
const TOPIC_LABELS = {
  basic: 'Basic Subtraction',
  negative: 'Negative Integers',
  multistep: 'Multi-step Problems',
  percent: 'Percentages',
  fraction: 'Fractions',
};
let topicStats = JSON.parse(localStorage.getItem('simsTopicStats') || '{}');
function saveTopicStats(){ localStorage.setItem('simsTopicStats', JSON.stringify(topicStats)); }
function recordTopicAttempt(key, isCorrect){
  if(!key) return;
  topicStats[key] = topicStats[key] || {attempts:0, correct:0};
  topicStats[key].attempts++;
  if(isCorrect) topicStats[key].correct++;
  saveTopicStats();
  syncTopicToClass(key, isCorrect);
}
async function syncTopicToClass(key, isCorrect){
  if(!activeClassCode || !window.firebase || !firebase.apps.length) return;
  const studentId = localStorage.getItem("simsStudentId"); if(!studentId) return;
  try{
    const tref = db().ref("classes/"+activeClassCode+"/students/"+studentId+"/topics/"+key);
    await tref.transaction(t=>{
      t = t || {attempts:0, correct:0};
      t.attempts = (t.attempts||0) + 1;
      if(isCorrect) t.correct = (t.correct||0) + 1;
      return t;
    });
  }catch(e){ console.warn("Topic sync was not saved", e); }
}
function renderWeakTopics(){
  const el = document.getElementById('weakTopicsPanel');
  if(!el) return;
  const MIN_ATTEMPTS = 3;
  const WEAK_THRESHOLD = 70;
  const entries = Object.keys(topicStats).map(key=>{
    const s = topicStats[key];
    return {key, label: TOPIC_LABELS[key] || key, attempts: s.attempts, correct: s.correct, pct: s.attempts ? Math.round(s.correct / s.attempts * 100) : 0};
  }).filter(t=>t.attempts >= MIN_ATTEMPTS).sort((a,b)=>a.pct-b.pct);

  if(!entries.length){
    el.innerHTML = `<h3>🔍 Weak Topics</h3><p class="weak-topics-empty">Keep practicing! Once you've answered a few questions in each topic, SIMS will highlight where you need the most practice.</p>`;
    return;
  }

  const weak = entries.filter(t=>t.pct < WEAK_THRESHOLD).slice(0,3);
  const summary = weak.length
    ? `<p class="weak-topics-summary">You're struggling most with <strong>${weak.map(w=>w.label).join(', ')}</strong>. Keep practicing to improve!</p>`
    : `<p class="weak-topics-summary">🎉 No weak spots right now — great work across every topic!</p>`;

  const rows = entries.map(t=>`
    <div class="weak-topic-row ${t.pct < WEAK_THRESHOLD ? 'weak' : ''}">
      <div class="weak-topic-top">
        <span class="weak-topic-name">${t.label}</span>
        <span class="weak-topic-meta">${t.correct}/${t.attempts} correct • ${t.pct}%</span>
      </div>
      <div class="weak-topic-bar"><i style="width:${t.pct}%"></i></div>
      ${t.pct < WEAK_THRESHOLD ? `<button class="weak-topic-btn" onclick="practiceTopic('${t.key}')">Practice this →</button>` : ''}
    </div>`).join('');

  el.innerHTML = `<h3>🔍 Weak Topics</h3>${summary}<div class="weak-topics-list">${rows}</div>`;
}
function practiceTopic(key){
  if(key === 'negative'){ startGame('advanced'); }
  else if(key === 'multistep'){ showQuiz(); setQuizDifficulty('difficult'); }
  else if(key === 'percent' || key === 'fraction'){ showQuiz(); setQuizDifficulty('advanced'); }
  else { startGame('beginner'); }
}
